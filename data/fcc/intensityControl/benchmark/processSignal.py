...
path = '/home/sivo/Documents/lcvision/40_simulations/fcc/intensityControl/benchmark/'
...
if setup:
    ...
    #bins_hw = 0.1
    #bins = np.arange(0.2, 30.0+0.01, 2*bins_hw)-bins_hw

    filename = '/home/sivo/Documents/lcvision/40_simulations/fcc/intensityControl/intensityControl_illya_spectrum.h5'

    with h5py.File(filename, 'r') as f:
        weight_illya = np.array(f.get('weight0'))
        photonEnergy_illya =np.array(f.get('energy_deviation')) / 1e9
    
    fig, ax = plt.subplots(figsize=(6,4))

    n_norm,bins,_ = ax.hist(photonEnergy_illya, bins, weights=weight_illya/N_e, histtype='step')
